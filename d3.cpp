...
int main() {
    ...
    IMAGE map;
    loadimage(&map, L"resource\\mapnew.png");
        ...
        while (HP > 0)
        {
            ...
            for (int i = 0; i < Enemynumber; i++)
            {
                if (level == 1) {
                    ...
                    if (enemy[i].x < player_x) {
                        loadimage(&enemyImg, L"resource\\enemy3.png");
                    }
                    else {
                        loadimage(&enemyImg, L"resource\\enemy4.png");
                    }
                    drawenemy(enemy[i].x, enemy[i].y, CameraPos.x, CameraPos.y, &enemyImg);
                }
                else if (level == 3) {
                    if (enemy[i].x < player_x) {
                        loadimage(&enemyImg, L"resource\\enemy5.png");
                    }
                    else {
                        loadimage(&enemyImg, L"resource\\enemy6.png");
                    }
                    drawenemy(enemy[i].x, enemy[i].y, CameraPos.x, CameraPos.y, &enemyImg);
                }
            }
